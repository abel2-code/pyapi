...
def byName(characterbio):
    if isinstance(characterbio[0], str):
        return characterbio[0]

simpsonsAge = sorted(simpsons, key=byAge)
simpsonsName = sorted(simpsons, key=byName)

print('Result of sorted(simpsons, key=byAge): ', simpsonsAge)
print('Result of sorted(simpsons, key=byName): ', simpsonsName)
